...
# 1. 頁面基本設定
st.set_page_config(page_title="精準資產計算機", layout="wide")

# 2. 初始化預設值與重設邏輯
def init_values():
    if 'init_cap' not in st.session_state: st.session_state.init_cap = 1000000
    if 'mon_inv' not in st.session_state: st.session_state.mon_inv = 10000 # 初始值 10,000
...
# --- B. 中間參數控制區 (帶有 +/- 按鈕，支援手動輸入) ---
st.subheader("⚙️ 調整投資參數")
c1, c2 = st.columns(2)
with c1:
    # st.number_input 預設即有加減按鈕，且點擊數字可直接打字
